...
| `scripts/deploy.sh`, `rollout.sh`, `build*.sh`, `sync-datasets.sh`, `install-git-hooks.sh`, `check-worktree-paths.sh`, `check-siblings.sh`, `check-duplication.py`, `check-example-scans.py`, `scripts/lib/**` | `docs/project-spec.md`, `README.md` | operational procedures, which manifests are generated vs committed, what the preflights check and when they stay silent |
...
| `url-fetcher/**` | `docs/code-execution-security.md` | the guard's refusal order and what makes its host allow-list configuration, per-hop re-validation and IP pinning, the `/fetch` request and response shape and its error types, what this pod deliberately holds none of |
| `url-fetcher/**` | `docs/project-spec.md`, `README.md` | the build context, its two entrypoints and the build-time gate that keeps the permissive one out of the image, which scripts build and roll it |
| `k8s/deployments/url-fetcher.yaml`, `k8s/network-policies/url-fetcher-policy.yaml` | `docs/code-execution-security.md` | the namespace's *second* egress policy: that the ingress allow-list is the access control on a deliberately unauthenticated route, the `except` blocks and the DNS rule the sandbox is refused, the KSA with no GCP binding and no mounted token |
...
| `scripts/test-network-policies.py` | `docs/code-execution-security.md` | the controls this harness is cited as enforcing — the sandbox's ingress/egress allow-lists, layer 2 of the MCP exclusion (layers 1 and 3 live in genetics-mcp-server's tests, not here), the `SANDBOX_ENABLED` pairing, which pod-spec fields are still treated as sandbox tells, and the url-fetcher's half: who may reach it, its egress shape, and the credential-free assertions an entry in `DEDICATED_SA_WORKLOADS` buys |
...
| `scripts/test-url-fetcher.py`, `scripts/external-inputs-proving-ground.py` | `docs/code-execution-security.md` | for `test-url-fetcher.py`: the mutation anchors driven against an in-process mutant of the module source, and the rule that an anchor which stops matching exits 2 rather than passing quietly; for `external-inputs-proving-ground.py`: the shared 0/1/2 exit-code convention, its precondition that the harness runs against a byte-identical supervisor, and which check groups it runs live versus reports NOT MEASURED |
