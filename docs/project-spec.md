...
something to fix in the file, and there is one outstanding: `plots.pyi` promises
`max_intersections: int = _UPSET_MAX_INTERSECTIONS` against a name that appears nowhere
in the shipped tree, because the generator copies a signature's default as source text
