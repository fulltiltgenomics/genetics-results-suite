...
- **results-api and bff ingress is source-scoped** (`k8s/network-policies/policies.yaml`): results-api (4000) admits exactly auth-gateway (its `@api_bearer` location), bff (the browser `/api/` path), chat-backend and mcp-server; bff (5000) admits only auth-gateway. Both previously carried a rule with `ports` and no `from:`, which in NetworkPolicy semantics admits every pod in the namespace — that was half of `genetics-results-suite-fad`. The monitor keeps its own additive rule in `monitor-policy.yaml`. Neither needs an `ipBlock`: both are ClusterIP with no Ingress backend, and kubelet probes are exempt from NetworkPolicy on this cluster (Dataplane V2). The remaining `from`-less rules (frontend, chat-backend, mcp-server, auth-gateway) are tracked as `genetics-results-suite-k4t`.
