...
- **Container privileges**: `allowPrivilegeEscalation: false`, `capabilities.drop: ["ALL"]` and the `RuntimeDefault` seccomp profile are set on **the containers of the suite's own services** — `results-api`, `chat-backend`, `mcp-server`, `bff`, `db-api`, and (since `genetics-results-suite-eau`) both containers of `auth-gateway`. The dynamically-created **sandbox** pods are hardened further still — `runAsNonRoot`, uid 65532, `allowPrivilegeEscalation: false`, `readOnlyRootFilesystem` (see `docs/code-execution-security.md` § 2) — and have no manifest anywhere in `k8s/`, so no grep over this repo will ever surface them. The controls are **not** set on the third-party and support workloads: `frontend`, `oauth2-proxy`, `keycloak`, `postgres`, `rag-service`, and the `monitor`, `analyze-conversations` and `keycloak-postgres-backup` CronJobs — **eight** workloads — all still run with the container defaults. Do not restate this as "every container". `grep -rn allowPrivilegeEscalation k8s/` (**`k8s/`, not `k8s/deployments/`**: the backup CronJob lives in `k8s/cronjobs/`, and a `k8s/deployments/`-scoped grep is exactly how it got left off this list once already) re-derives only the **hardened** side. The non-hardened side is that set's **complement**, so it cannot be grepped for at all — derive it by listing every workload manifest in `k8s/` and subtracting the hardened ones.
  - `db-api` and `bff` additionally run `runAsNonRoot` (uid 10001 / 1000) since they write nothing outside their image; results-api, chat-backend and mcp-server still run as root because they raise `ulimit`, shell out to `gcloud`, cache tabix indexes, or own root-owned files on the `chat-data` PVC. chat-backend sets `fsGroup: 1032` so the pre-existing SQLite files stay writable once `CAP_DAC_OVERRIDE` is dropped.
  - **auth-gateway is the one exception to a bare `drop: ["ALL"]`**, and the additions are load-bearing rather than cautious. `nginx:1.27-alpine`'s master starts as root and its build defaults to `--user=nginx` (`nginx -V` → `--user=nginx --group=nginx`), which the mounted config does not override, so startup chowns `/var/cache/nginx/*_temp` to uid 101 and each worker `setgid`/`setuid`s to it. Measured against the image under `docker --cap-drop=ALL --security-opt=no-new-privileges`: bare drop-ALL aborts at `chown("/var/cache/nginx/client_temp", 101) failed (1: Operation not permitted)`; drop-ALL plus only `CHOWN` starts the master but leaves `setgid(101) failed` with the worker "exited with fatal code 2 and cannot be respawned" — a master holding :8080 and answering nothing, i.e. a liveness-probe restart loop on the internet-facing pod. The nginx container therefore adds `CHOWN`, `SETUID`, `SETGID`; the `render-config` initContainer adds only `CHOWN`, because its `nginx -t` is not a dry run (it creates and chowns the same temp paths) but never forks a worker. `NET_BIND_SERVICE` is deliberately **absent**: the only `listen` is 8080, and neither fragment `deploy.sh` injects (`${LEGACY_REDIRECT}`, `${KEYCLOAK_SERVER}`) adds a listener. Running the pod as uid 101 instead would drop all three **and make the master non-root**, which is the part that actually matters; it needs `/var/cache/nginx` made writable (an emptyDir the pod does not have) — filed as future work, not done here.
  - **What the auth-gateway hardening buys, measured — and what it does not.** The nginx *worker*, the process that parses internet traffic, was **already** fully unprivileged: `setuid(101)` without `PR_SET_KEEPCAPS` clears the capability sets, so today's live pod with an empty `securityContext` shows Uid 101, CapPrm 0, CapEff 0 — identical to the hardened pod. What the change hardens is the **master**, which never touches attacker input: CapEff `0xa80425fb` (the 14-cap runtime default) → `0xc1`, and NoNewPrivs 0 → 1. That is a real gain — it removes `DAC_OVERRIDE`, `NET_RAW`, `MKNOD`, `SYS_CHROOT`, `SETPCAP`, `SETFCAP`, `KILL`, `FOWNER`, `FSETID` and `AUDIT_WRITE`, shortening the post-exploitation chain — but it does **not** reduce the privileges an nginx RCE would land in, and the master is still uid 0 on a writable rootfs. The gateway is not meaningfully harder to exploit; it is less useful once exploited.
