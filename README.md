...
- The suite's own service containers — results-api, chat-backend, mcp-server, bff, db-api and both auth-gateway containers — run with `allowPrivilegeEscalation: false`, capabilities dropped and the `RuntimeDefault` seccomp profile; db-api and bff additionally run as non-root. auth-gateway keeps `CHOWN`/`SETUID`/`SETGID` (nginx's root master setuids its workers to uid 101 — a bare drop-ALL crash-loops it). Eight third-party and support workloads (frontend, oauth2-proxy, keycloak, postgres, rag-service, and the monitor, analyze-conversations and keycloak-postgres-backup CronJobs) are not hardened this way; the dynamically-created sandbox pods go further still (non-root uid 65532, read-only rootfs). The auth-gateway change hardens nginx's root *master* process, not its already-unprivileged worker. See `docs/project-spec.md` → Security.
