...
silently regenerate the shipped stubs from an old SDK.
...
- Every workload under `k8s/` sets a `securityContext`; none runs with the container defaults.
  ...
  **generated from the manifests** by `scripts/gen-doc-blocks.py`, so re-read it rather than a
  list here. The shape of the decisions behind it: the baseline everywhere is
  `allowPrivilegeEscalation: false`, drop-`ALL` with nothing added back, and `RuntimeDefault`
  seccomp; `runAsNonRoot` and `readOnlyRootFilesystem` were added one image at a time, measured
  under the constraint rather than applied as a uniform block, because several images needed
  something specific (an `emptyDir` for a cache or a unix socket, an `fsGroup` matching an
  existing PVC's ownership, or — for the backup CronJob — capabilities its run-time
  `apt-get install` genuinely needs). No workload in the namespace mounts the `default`
  ServiceAccount token any more. The sandbox goes furthest, and its manifest and reasoning are
  `k8s/deployments/sandbox.yaml` and `docs/code-execution-security.md`.
