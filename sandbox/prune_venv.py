...
SDK_ALLOWLIST = frozenset(
    {
        ...
        "sdk/client.py",
        "sdk/errors.py",
        # NOT in the import closure — sdk/__init__.py resolves plots through a module
        # __getattr__ so the servers never import matplotlib, and phewas_categories is
        # imported only by plots — so these two survive on this list alone. They are the
        # entries whose absence would be a missing FEATURE rather than a broken import:
        # without them `genetics.plots` raises ModuleNotFoundError inside the child, where
        # the standard plots are the whole point of shipping it.
        "sdk/phewas_categories.py",
        "sdk/plots.py",
        "tools/__init__.py",
        "tools/executor.py",
        "tools/sql_safety.py",
